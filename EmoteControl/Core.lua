...
-- Initialize the EmoteControl namespace
...
taintFrame:SetScript("OnEvent", function(_, event, addonName, funcName)
  if addonName == "EmoteControl" or addonName == ADDON_NAME then
    local sourceLine
...
function addon:LoadPacksForPlayer()
  -- Option B loader:
  -- Load every enabled addon whose folder name starts with "EmoteControl_Pack_"
  ...
  -- (Legacy keys with X-EmoteControl-* are also supported.)
...
local function RegisterSlashCommands()
  ...
  if rawget(_G, "SLASH_EmoteControl1") == nil then
    rawset(_G, "SLASH_EmoteControl1", "/sl")
  end
  if rawget(_G, "SLASH_EmoteControl2") == nil then
    rawset(_G, "SLASH_EmoteControl2", "/EmoteControl")
  end
  if type(SlashCmdList["EmoteControl"]) ~= "function" then
    SlashCmdList["EmoteControl"] = HandleSlash
  end
