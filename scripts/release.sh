...
if command -v luac >/dev/null; then
  ...
  echo "Warning: luac not found; skipping Lua syntax validation." >&2
fi

git add SpeakinLite/Core.lua SpeakinLite/SpeakinLite.toc CHANGELOG.md
...
if not id_12:
    print("Failed to resolve game version ID for 12.x.", file=sys.stderr)
    sys.exit(1)
print(f"{id_12}")
PY
)"

gv_12="$game_version_ids"

metadata_json="$(
  VERSION="$VERSION" GV12="$gv_12" python3 - <<'PY'
import json,os,re
...
meta={
    ...
    "gameVersions": [int(os.environ["GV12"])],
}
